...
def cifar10_bayes_learn(Xf, Y, sz):
...
def cifar10_classifier_bayes(x,mu,sigma,p):
    ...
    for i in range(NUM_OF_CLASS):
        prob[i] = scipy.stats.multivariate_normal.logpdf(x,mu[i],sigma[i])
    return np.argmax([prob[i]+ math.log(p[i]) for i in range(NUM_OF_CLASS)])
...
acc = []
for sz in n :
    tempdataX = cifar10_color(dataX,sz)
    tempX = cifar10_color(X,sz)
    mu, sigma, prior = cifar10_bayes_learn(tempdataX,dataY,sz)
    pred = [cifar10_classifier_bayes(x,mu,sigma,prior) for x in tempX]
    acc.append(class_acc(pred,Y))
#expect [0.2479,0.3105,0.4021, 0.4173 , (n > 8 then execute time is really long and I dont have time to run it :<)]
print(acc)
plt.plot(n,acc)